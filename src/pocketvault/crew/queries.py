from pocketvault.database import get_connection

def get_pocket_balances(db_path: str) -> list[dict]:
    """Get all pockets with balances from Crew API."""
    conn = get_connection(db_path)
    rows = conn.execute("SELECT * FROM pocket_balances").fetchall()
    conn.close()
    return [dict(row) for row in rows]

def get_ready_to_budget(db_path: str) -> float:
    """Get Autopilot Reserve balance from API (ready to budget)."""
    conn = get_connection(db_path)
    row = conn.execute("SELECT balance_cents FROM pockets WHERE name = 'Autopilot Reserve' AND active = 1").fetchone()
    conn.close()
    return (row["balance_cents"] or 0) / 100.0 if row else 0.0

def get_total_crew_balance(db_path: str) -> float:
    """Get total balance across all active pockets from API."""
    conn = get_connection(db_path)
    row = conn.execute("SELECT SUM(balance_cents) as total FROM pockets WHERE active = 1").fetchone()
    conn.close()
    return (row["total"] or 0) / 100.0

def get_entries_for_pocket(db_path: str, pocket_name: str, limit: int = 100) -> list[dict]:
    """Get recent entries for a pocket."""
    conn = get_connection(db_path)
    rows = conn.execute("""
        SELECT e.* FROM entries e
        JOIN pockets p ON p.id = e.pocket_id
        WHERE p.name = ?
        ORDER BY e.timestamp DESC
        LIMIT ?
    """, (pocket_name, limit)).fetchall()
    conn.close()
    return [dict(row) for row in rows]
